...
export class NvidiaAgent extends BaseAgent {
  constructor(config: AgentConfig) {
    super(config);
  }

  async getCompletion(request: CompletionRequest): Promise<CompletionResponse> {
    const model = request.model || this.config.freeModels[0] || 'meta/llama-3.1-8b-instruct';
    const body = {
      model,
    ...
    if (result.ok) {
      updateQuotaFromHeaders(this.state, {});
    }
  ...
  async healthCheck(): Promise<boolean> {
    ...
    try {
      const result = await httpPost(
        `${this.config.baseUrl}/chat/completions`,
        {
          model: this.config.freeModels[0] || 'meta/llama-3.1-8b-instruct',
