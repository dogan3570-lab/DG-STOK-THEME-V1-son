...
const SAFE: Record<string, number> = { '2876': 4956 /*Tablet Standı*/, '1650': 4763 /*Kitap Tutucu*/ };
async function main() {
  ...
  for (const [k, ext] of Object.entries(SAFE)) {
    ...
    if (MODE === 'apply') {
      ...
      await prisma.auditLog.create({ data: { action: 'CATEGORY_MATCH', entity: 'category', entityId: cat.id, details: `[mr34-resolved] ${k} -> ${cat.name} (ext=${ext})`, meta: JSON.stringify({ productId: p.id, xmlKey: k, method: 'image_verified_single_leaf', scope: 'PRODUCT', visualVerified: true, newCategoryId: cat.id }) } });
      applied++;
    }
  }
