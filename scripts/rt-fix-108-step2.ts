// scripts/rt-fix-108-step2.ts
// Phase 1: Verify pricing rule + 21 price products
// Phase 2: Check TPA status of 87 attribute products, run autoResolve for missing
// Phase 3: Full gate retest
...
async function main() {
  ...
  // PHASE 1: Verify pricing rule + 21 price products
  // ═══════════════════════════════════════════════════════════
  console.error('=== PHASE 1: Pricing Rule Verification ===');

  const rules = await prisma.marketplacePricingRule.findMany({
    where: { marketplaceId: MP, active: true },
    orderBy: { minPrice: 'asc' },
  });
  R['pricing_rules'] = rules.map(r => ({
    id: r.id.substring(0,8), min: r.minPrice, max: r.maxPrice, margin: r.profitMargin, fixed: r.fixedAmount,
  }));
  ...
  let pricePass = 0;
  const priceDetails: Array<{id:string; price:number; pass:boolean}> = [];
  for (const p of priceProducts) {
    ...
    if (pass) pricePass++;
    priceDetails.push({ id: p.id.substring(0,8), price: p.salePrice!, pass });
  }
  R['phase1_price'] = { passed: pricePass, total: 21, allPass: pricePass === 21 };
  console.error(`Phase 1: ${pricePass}/21 price products PASS ✓`);

  // ═══════════════════════════════════════════════════════════
  // PHASE 2: Check TPA status of 87 attribute products
  // ═══════════════════════════════════════════════════════════
  console.error('=== PHASE 2: TPA Status Check ===');

  const tpaBefore = await prisma.trendyolProductAttribute.findMany({
    where: { marketplaceKey: 'tt', productId: { in: ATTR_BLOCKED_IDS } },
    select: { productId: true, attributeId: true, attributeValueId: true },
  });
  const tpaBeforeMap = new Map<string, number>();
  for (const t of tpaBefore) {
    tpaBeforeMap.set(t.productId, (tpaBeforeMap.get(t.productId) || 0) + 1);
  }
  const beforeWithTpa = ATTR_BLOCKED_IDS.filter(id => (tpaBeforeMap.get(id) || 0) > 0).length;
  const beforeWithoutTpa = ATTR_BLOCKED_IDS.filter(id => (tpaBeforeMap.get(id) || 0) === 0).length;
  R['phase2_tpa_before'] = { withTpa: beforeWithTpa, withoutTpa: beforeWithoutTpa };
  console.error(`TPA before: ${beforeWithTpa} have records, ${beforeWithoutTpa} need records`);

  // Find which products need TPA
  const needTpa = ATTR_BLOCKED_IDS.filter(id => (tpaBeforeMap.get(id) || 0) === 0);
  R['phase2_need_tpa'] = { count: needTpa.length, ids: needTpa.map(id => id.substring(0,8)) };

  if (needTpa.length === 0) {
    console.error('All 87 products already have TPA records! Skipping auto-resolve.');
  } else {
    console.error(`Need TPA for ${needTpa.length} products. Running auto-resolve...`);

    // Use autoResolveMissingFields with smaller batches
    ...
    for (let i = 0; i < needTpa.length; i += BATCH) {
      const batch = needTpa.slice(i, i + BATCH);
      console.error(`  Batch ${Math.floor(i/BATCH)+1}/${Math.ceil(needTpa.length/BATCH)}: ${batch.length} products...`);

      const res = await autoResolveMissingFields({
        useAI: true,
        maxProducts: BATCH,
        batchSize: BATCH,
        waitMs: 500,
      ...
      totalDet += res.deterministic;
      totalAi += res.ai;
      totalUnresolved += res.unresolved;
      console.error(`  → processed=${res.processed} det=${res.deterministic} ai=${res.ai} unresolved=${res.unresolved}`);
    }

    R['phase2_auto_resolve'] = {
      totalProcessed, totalDet, totalAi, totalUnresolved,
    };
    console.error(`Auto-resolve complete: processed=${totalProcessed} det=${totalDet} ai=${totalAi} unresolved=${totalUnresolved}`);
  }

  // ═══════════════════════════════════════════════════════════
  // PHASE 2-VERIFY: TPA records after
  // ═══════════════════════════════════════════════════════════
  console.error('=== PHASE 2-VERIFY: TPA After ===');

  const tpaAfter = await prisma.trendyolProductAttribute.findMany({
    where: { marketplaceKey: 'tt', productId: { in: ATTR_BLOCKED_IDS } },
    select: { productId: true, attributeId: true, attributeValueId: true },
  });
  const tpaAfterMap = new Map<string, number>();
  for (const t of tpaAfter) {
    tpaAfterMap.set(t.productId, (tpaAfterMap.get(t.productId) || 0) + 1);
  }
  const afterWithTpa = ATTR_BLOCKED_IDS.filter(id => (tpaAfterMap.get(id) || 0) > 0).length;
  const afterWithoutTpa = ATTR_BLOCKED_IDS.filter(id => (tpaAfterMap.get(id) || 0) === 0);
  R['phase2_tpa_after'] = { withTpa: afterWithTpa, withoutTpa: afterWithoutTpa.length };
  console.error(`TPA after: ${afterWithTpa} have records, ${afterWithoutTpa.length} still missing`);

  if (afterWithoutTpa.length > 0) {
    R['phase2_still_missing'] = afterWithoutTpa.map(id => id.substring(0,8));
    console.error(`Still missing: ${afterWithoutTpa.map(id => id.substring(0,8)).join(', ')}`);
  }
  ...
  for (const r of allTpa) {
    if (!tpaSet.has(r.productId)) tpaSet.set(r.productId, new Set());
    tpaSet.get(r.productId)!.add(r.attributeId);
  }

  // Pricing rules (post new rule)
  ...
  const blockedIdsSet = new Set<string>();
  ...
  for (const p of allP) {
    ...
    if (!firstFailure) {
      const tpa = tpaSet.get(p.id) || new Set();
  ...
  R['phase3_retest'] = {
    total: allP.length, ready: readyNow, blocked: blockedNow,
    checksum: readyNow + blockedNow, checksumPass: readyNow + blockedNow === allP.length,
    blockedByCode,
  };
  console.error(`Retest: READY=${readyNow} BLOCKED=${blockedNow} TOTAL=${allP.length}`);

  // ═══════════════════════════════════════════════════════════
  // PHASE 4: 108-product cross-check
  // ═══════════════════════════════════════════════════════════
  const all108 = [...PRICE_BLOCKED_IDS, ...ATTR_BLOCKED_IDS];
  const verify108: Array<{id:string; group:string; nowPass:boolean}> = [];
  for (const id of PRICE_BLOCKED_IDS) {
    verify108.push({ id, group: 'PRICE', nowPass: readyIds.has(id) });
  }
  for (const id of ATTR_BLOCKED_IDS) {
    verify108.push({ id, group: 'ATTRIBUTE', nowPass: readyIds.has(id) });
  }

  const opened = verify108.filter(v => v.nowPass).length;
  const stillBlocked = verify108.filter(v => !v.nowPass);
  R['phase4_108'] = {
    total: 108, opened, stillBlocked: stillBlocked.length,
    allOpened: opened === 108,
    ...
    stillBlockedDetails: stillBlocked.map(v => ({
      id: v.id.substring(0,8), group: v.group,
    })),
  };
  ...
  // ═══════════════════════════════════════════════════════════
  R['summary'] = {
    before: { ready: 2353, blocked: 836 },
    after: { ready: readyNow, blocked: blockedNow },
    delta: { opened: readyNow - 2353, closed: 2353 + 836 - (readyNow + blockedNow) },
    price: { before: 21, opened: PRICE_BLOCKED_IDS.filter(id => readyIds.has(id)).length },
    attribute: { before: 87, opened: ATTR_BLOCKED_IDS.filter(id => readyIds.has(id)).length },
    category: { before: 728, after: blockedByCode['CATEGORY_MAPPING_NOT_FOUND'] || 0, same: (blockedByCode['CATEGORY_MAPPING_NOT_FOUND'] || 0) === 728 },
  };
