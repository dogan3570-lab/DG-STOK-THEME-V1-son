...
const SAFE: Record<string, number> = {
  '1427': 1863,  // Emzirme Örtüsü
  '2525': 1870, '2528': 1870, '2530': 1870, '2531': 1870, // Koruyucu ve Kilit
};
async function main() {
  ...
  for (const [k, ext] of Object.entries(SAFE)) {
    ...
    if (MODE === 'apply') {
      ...
      await prisma.auditLog.create({ data: { action: 'CATEGORY_MATCH', entity: 'category', entityId: cat.id, details: `[ie41-resolved] ${k} -> ${cat.name} (ext=${ext})`, meta: JSON.stringify({ productId: p.id, xmlKey: k, method: 'deep_tree_search', scope: 'PRODUCT', visualVerified: true, newCategoryId: cat.id }) } });
      applied++;
    }
  }
