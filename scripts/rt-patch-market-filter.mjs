// scripts/rt-patch-market-filter.mjs
import { readFileSync, writeFileSync } from 'node:fs';
const P = 'C:/PROJE 1/DG-STOK-THEME-V1/dist/index.html';
let c = readFileSync(P, 'utf8');
if (c.includes('sr-cat')) { console.log('already patched'); process.exit(0); }

// ---- 1) filters HTML: add sorun türü + kategori controls ----
const inpAnchor = '<input class="sr-search" id="sr-search" placeholder="Ürün adı / barkod ara..."/>';
if (!c.includes(inpAnchor)) { console.log('input anchor NOT FOUND'); process.exit(1); }
const added = inpAnchor +
  '<span id="sr-rtype-wrap" style="display:none;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-size:11px;color:var(--sr-muted);font-weight:700">SORUN TÜRÜ:</span><span id="sr-rtypes" style="display:flex;gap:6px;flex-wrap:wrap"></span></span>' +
  '<select id="sr-cat" style="border:1px solid var(--sr-line);border-radius:10px;padding:8px 12px;font-size:13px;background:#fff;min-width:190px"><option value="">Tüm kategoriler</option></select>';
c = c.replace(inpAnchor, added);

// ---- 2) script: replace whole sr-script body ----
const start = c.indexOf('<script id="sr-script">');
const end = c.indexOf('</script>', start);
if (start < 0 || end < 0) { console.log('sr-script NOT FOUND'); process.exit(1); }

const SCRIPT = `<script id="sr-script">
(function(){
  var ALL=[],MPS=[],MP='',RF='',RTYPE='',RCAT='',SEARCH='';
  ...
  function mp(){var nm={};MPS.forEach(function(m){nm[m.key]=m.name});
    document.getElementById('sr-mpgrid').innerHTML=MPS.map(function(m){var c=mpCnt(m.key);return '<div class="sr-mp'+(MP===m.key?' sr-active':'')+'" data-mp="'+esc(m.key)+'"><div class="sr-nm">'+esc(m.name)+'<span class="sr-k">'+fmt(c.approved+c.pending+c.rejected)+' \\u00fcr\\u00fcn</span></div><div class="sr-stats"><div class="sr-st"><div class="sr-v" style="color:var(--sr-green)">'+fmt(c.approved)+'</div><div class="sr-t">Ba\\u015far\\u0131l\\u0131</div></div><div class="sr-st"><div class="sr-v" style="color:var(--sr-amber)">'+fmt(c.pending)+'</div><div class="sr-t">Bekleyen</div></div><div class="sr-st"><div class="sr-v" style="color:var(--sr-red)">'+fmt(c.rejected)+'</div><div class="sr-t">Red</div></div><div class="sr-st"><div class="sr-v" style="color:var(--sr-blue)">'+fmt(c.action)+'</div><div class="sr-t">Aksiyon</div></div></div></div>';}).join('');
    document.querySelectorAll('#sr-mpgrid .sr-mp').forEach(function(el){el.onclick=function(){var k=el.getAttribute('data-mp');MP=(MP===k?'':k);RTYPE='';RCAT='';mp();refreshFilters();render();};});
  }
  // Seçilen pazaryerine göre sorun türü chips + kategori seçenekleri (yalnız o pazaryerinin gerçek ürünleri)
  ...
  function refreshFilters(){var sc=mpScope();
    ...
    var sel=document.getElementById('sr-cat');
    sel.innerHTML='<option value="">Tüm kategoriler</option>'+Object.keys(cats).map(function(id){return '<option value="'+esc(id)+'"'+(RCAT===id?' selected':'')+'>'+esc(cats[id])+'</option>';}).join('');
  }
  ...
  window.srLoad=async function(){try{var r=await fetch('/api/ready-to-ship/results',{credentials:'include'});
    ...
    var j=await r.json();ALL=(j.items||[]);MPS=(j.marketplaces||[]);sum();chart();mp();refreshFilters();issues();actions();render();
