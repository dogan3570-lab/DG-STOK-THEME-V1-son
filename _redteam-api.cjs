...
// SADECE OKUMA: login dışında veri değiştirmez.
...
(async () => {
  const login = await call('POST', '/auth/login', { body: { email: 'admin@dgstok.com', password: 'admin123' } });
  const token = login.body && login.body.token;
  ok('POST /auth/login -> token', login.status === 200 && !!token, 'status=' + login.status);
  ...
  ok('Product Pool pendingCategory (DB)', stats.status === 200, 'pendingCategory=' + (stats.body && stats.body.pendingCategory) + ' total=' + (stats.body && stats.body.totalProducts));

  const catStats = await call('GET', '/categories/stats' + srcQ, { token });
  ok('Category stats uncategorizedProducts (DB)', catStats.status === 200, 'uncategorized=' + (catStats.body && catStats.body.unmatchedProducts));
  ...
  ok('Variant dashboard (DB)', dash.status === 200, JSON.stringify(dash.body));
  ...
  ok('Variant stats (DB)', varStats.status === 200, JSON.stringify(varStats.body));

  // Tam kategori kümesinde categoryId null sayısını hesapla (API seviyesinde)
  if (catLen === catTotal) {
