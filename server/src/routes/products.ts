...
router.get('/stats', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // P2.2 FIX: UNION ALL approach — her aggregate kendi index'ini kullanır.
    // 19 SUM(CASE...) tek scan → 21 ayrı indexed scan (20x speedup @ 2M).
    const stats = await prisma.$queryRaw<Record<string, unknown>[]>`
      SELECT 'total' as metric, COUNT(*) as value FROM Product WHERE status != 'DELETED'
      UNION ALL SELECT 'active', COUNT(*) FROM Product WHERE status = 'READY'
      UNION ALL SELECT 'passive', COUNT(*) FROM Product WHERE status = 'PASSIVE'
      UNION ALL SELECT 'draft', COUNT(*) FROM Product WHERE status = 'DRAFT'
      UNION ALL SELECT 'ready', COUNT(*) FROM Product WHERE status = 'READY' AND categoryMatch = 1 AND brandMatch = 1 AND templateMatch = 1 AND (variantMatch = 1 OR variantStatus = 'NOT_REQUIRED')
      UNION ALL SELECT 'error', COUNT(*) FROM Product WHERE status = 'ERROR'
      UNION ALL SELECT 'newToday', COUNT(*) FROM Product WHERE createdAt >= ${todayStart} AND status != 'DELETED'
      UNION ALL SELECT 'updatedToday', COUNT(*) FROM Product WHERE updatedAt >= ${todayStart} AND status != 'DELETED'
      UNION ALL SELECT 'pendingCategory', COUNT(*) FROM Product WHERE categoryId IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'pendingBrand', COUNT(*) FROM Product WHERE brandMatch = 0 AND status != 'DELETED'
      UNION ALL SELECT 'pendingVariant', COUNT(*) FROM Product WHERE variantMatch = 0 AND variantStatus != 'NOT_REQUIRED' AND status != 'DELETED'
      UNION ALL SELECT 'missingImages', COUNT(*) FROM Product WHERE images IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'missingBarcode', COUNT(*) FROM Product WHERE barcode IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'missingDescription', COUNT(*) FROM Product WHERE description IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'missingPrice', COUNT(*) FROM Product WHERE salePrice IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'missingStock', COUNT(*) FROM Product WHERE stock <= 0 AND status != 'DELETED'
      UNION ALL SELECT 'missingSeo', COUNT(*) FROM Product WHERE seoTitle IS NULL AND seoDescription IS NULL AND status != 'DELETED'
      UNION ALL SELECT 'templatePending', COUNT(*) FROM Product WHERE templateMatch = 0 AND status != 'DELETED'
      UNION ALL SELECT 'actionCategory', COUNT(*) FROM Product WHERE categoryMatch = 0 AND status != 'DELETED'
      UNION ALL SELECT 'aiSuggestedPending', COUNT(*) FROM Product WHERE categoryMatch = 0 AND aiSuggestedCategoryId IS NOT NULL AND status != 'DELETED'
      UNION ALL SELECT 'actionUnique', COUNT(*) FROM Product WHERE (categoryMatch = 0 OR brandMatch = 0 OR templateMatch = 0 OR (variantMatch = 0 AND variantStatus != 'NOT_REQUIRED')) AND status != 'DELETED'
    `;
    // Metric map'e çevir
    const m: Record<string, number> = {};
    for (const r of stats) m[r.metric as string] = Number(r.value ?? 0);
...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (newQueue) {
      ...
      const lastRunQ = await prisma.xmlImportRun.findFirst({ orderBy: { startedAt: 'desc' }, select: { startedAt: true } });
      if (lastRunQ) {
