...
// minStock query param: stock >= minStock filter. "all" disables it (returns everything).
// Param absent → no stock filter (backward compatible for other consumers).
router.get('/:id/products', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const rawMinStock = String(req.query?.minStock ?? '').trim();
    const hasMinStockParam = rawMinStock !== '' && rawMinStock.toLowerCase() !== 'all';
    ...
    if (hasMinStockParam) {
      const minStock = Math.max(0, Number(rawMinStock) || 0);
