...
export async function reconcileReadiness(productId: string, ctx: ReconcileContext): Promise<boolean> {
  ...
  // P0 DEFECTIVE GATE (tek gerçek kaynak): resim/barkod/tanım/fiyat/stok — bu koşul
  // /products/stats'taki "defective" metriği ve /products?defective=true filtresiyle
  // BİREBİR AYNI koşuldur. Eksik ürün READY'e promote edilemez; mevcut READY'de
  // eksik veri tespit edilirse demote edilir (kural ihlali kalmaz).
  ...
  const shouldBeReady = hasPms && allGates && hasCompleteData && stockEligible && hasActiveMapping;
  ...
  while (true) {
    ...
    await prisma.$transaction(async (tx) => {
      for (const p of products) {
        ...
        // P0 DEFECTIVE GATE (tek gerçek kaynak): tekil reconcile ile birebir aynı koşul
        const hasCompleteData = !!(
          p.salePrice != null &&
          (p.stock ?? 0) > 0 &&
          p.images != null &&
        ...
        const shouldBeReady = allGates && hasCompleteData && stockEligible && activeMapping; // PMS already guaranteed by the query
