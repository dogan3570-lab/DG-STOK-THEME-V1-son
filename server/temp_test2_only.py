...
# Get test 2's product
db = sqlite3.connect(r'C:\PROJE 1\DG-STOK-THEME-V1\server\prisma\dev.db')
rows = db.execute("SELECT id, title, supplierCategory FROM Product WHERE categoryMatch = 0 AND xmlKey IS NOT NULL LIMIT 10").fetchall()
db.close()

# Show all products
for i, r in enumerate(rows):
    print(f'  [{i}] {(r[1] or "?")[:60]} | supplierCategory={(r[2] or "?")[:40]}')

# Test product 1 (index 1 = "Pratik Ipli Mutfak...")
test_id = rows[1][0]
test_title = rows[1][1] or 'unknown'
print(f'\n=== SINGLE TEST: {test_title[:50]} ===')
print(f'Product ID: {test_id}')

try:
    s, d = api('POST', '/category-engine/run', {'productIds': [test_id]})
    print(f'HTTP {s}')
